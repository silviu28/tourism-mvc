...
import { addNotificationSchema, updateNotificationSchema } from "../schemas";

const router = express.Router();

router.get("/api/notifications", adminTokenAuthenticator, async (req, res) => {
  return handlePagedQuery(Notification, req, res);
});
...
router.post("/api/notifications", adminTokenAuthenticator, async (req, res) => {
  try {
    const parsed = addNotificationSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: parsed.error.flatten() });
    }

    const { title, content, category, duration } = parsed.data;
